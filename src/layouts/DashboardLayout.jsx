...
export default function DashboardLayout() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      {/* Sidebar */}
      <aside
        className={`bg-white shadow-xl w-64 flex-shrink-0 z-20 transition-all duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full fixed h-full'
          }`}
      >
            ...
            {navigation.map((item) => (
              ...
              >
                {({ isActive }) => (
                  <>
                    <item.icon
                      className={`mr-3 h-5 w-5 ${isActive ? "text-[#013b82]" : "text-slate-400"
                        }`}
                    />
                    {item.name}
                  </>
